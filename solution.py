...
class RubiksCube:
    ...
    def apply_move(self, move):
        # Simplified version: actual move rotations need to be implemented
        # Here we just track moves to simulate structure
        pass  # TODO: implement real face rotations
...
def heuristic(cube):
    # Count number of misplaced stickers (simplified)
    cost = 0
    for face in FACES:
        cost += sum(1 for s in cube.state[face] if s != face)
    return cost // 8  # a rough approximation
...
def ida_star(cube):
    ...
    def search(node, g, prev_move, threshold):
        ...
        for move in MOVES:
            if move == prev_move:  # avoid repeating same move
                continue
            new_node = node.copy()
